...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> _savePassword() async {
    ...
    if (old.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('أدخل كلمة المرور الحالية')));
      return;
    }
    ...
    if (pw.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('الرجاء إدخال كلمة المرور الجديدة')));
      return;
    }
    if (pw.length < 6) {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) {
        return AlertDialog(
          ...
          content: const Text('هل أنت متأكد من حذف الحساب؟ لا يمكن التراجع عن هذه العملية.'),
          actions: [
